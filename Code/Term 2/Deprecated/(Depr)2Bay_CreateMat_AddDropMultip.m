...
while ~isempty(queue)
        ...
        if s(1)==2 
            for t1 = 0:max_H
                for t2 = t1:max_H
                    ...
                    succs(end+1,:) = {new_s, [t1 t2]}; %adding successor along with height thrown to reach position (t in this case)
                end
            end
    ...
    for j = 1:size(succs, 1) %going through rows of succs matrix that contains successor state and throw height (at end)
        new_s = succs(j, 1:max_H); %successor extracted
        h = succs(j, max_H+1); %throw height extracted
...
transition_values = -ones(state_count,state_count); %Matrix representing throw heights associated with moving from state i to state j
