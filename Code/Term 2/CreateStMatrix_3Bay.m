...
while ~isempty(queue)
    ...
    if s(1) == 0
        ...
        if sum(s) + 3 <= max_b %only gets started for state 000
            for i1 = 1:max_H
                for i2 = i1:max_H
                    for i3 = i2:max_H
                    new_s = s;
                    ...
                    new_s(i2) = new_s(i2) + 1; %add new ball to the system
                    new_s(i3) = new_s(i3) + 1;
                    succs(end+1,:) = {new_s, [0,0,0]}; %new state added along with throw height (0 in this case as ball is added at any point in the state)
                    end
                end
            end
        end
        ...
        if s(1)==2 
            for t1 = 0:max_H
                for t2 = t1:max_H
                    ...
                    % new_s(1) = new_s(1) + (s(1) - 2); %s(1)-2=0 as only case where s(1)==2 is available here
            ...
            for t1 = 0:max_H
                    for t2 = t1:max_H
                        for t3 = t2:max_H
                            ...
                            % new_s(1) = new_s(1) + (s(1) - 3);  %no ball gets pushed forward as s(1)-3 = 0 
                        ...
                        %ones(0) (s(1)==2) is empty ones(1) (s(1)==3) is 1 so
                        %both cases are handled
                        succs(end+1,:) = {new_s, [t1 t2,t3]}; %adding successor along with height thrown to reach position (t in this case)
                        end
                    end
            end
        else %s(1)==1 / 
